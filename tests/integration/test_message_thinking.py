...
def test_get_messages_returns_thinking_content(client, admin_token, integration_db):
    ...
    with get_db_ctx() as session:
        ...
        session.add(Conversation(
            conversation_id=conv_id, user_id=admin_id, title="thinking 测试",
            created_at=utc_now(), updated_at=utc_now(),
        ))
        session.add(Message(
            message_id=new_id(), conversation_id=conv_id, user_id=admin_id,
            role="assistant", content="答案正文",
    ...
    resp = client.get(
        f"/api/v1/chat/conversations/{conv_under_test}/messages", headers=headers,
    )
    assert resp.status_code == 200, resp.text
    msgs = resp.json()
    assert len(msgs) == 1
