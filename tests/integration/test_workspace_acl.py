...
async def test_restricted_doc_visible_to_owner_only(integration_db, fake_llm_stack):
    ...
    with get_db_ctx() as session:
        ...
        for uid in ("alice", "bob"):
            if not session.query(User).filter_by(id=uid).first():
                session.add(User(
                    id=uid, username=uid, hashed_password="unused", is_active=True))
        session.flush()
        if not session.query(KnowledgeBase).filter_by(id="alice-ws").first():
            session.add(KnowledgeBase(
                id="alice-ws", name="Alice 的工作空间",
                visibility="restricted", owner_id="alice"))
        session.commit()
    ...
    with get_db_ctx() as session:
        session.add(Document(
            document_id=doc_id, kb_id="alice-ws", filename="private.md",
            owner_id="alice", status="processing",
    ...
    res = await asyncio.to_thread(
        ...
        "private.md", content.encode("utf-8"), kb_id="alice-ws",
        user_id="alice", visibility="restricted", allowed_roles=[],
        document_id=doc_id)
    ...
    async def _retrieve(uid: str, can_read_all: bool = False):
        return await retrieval_engine.retrieve(
            "青龙计划 机密预算", None,
            user_role_ids=[], can_read_all=can_read_all, user_id=uid)
    ...
    assert any(c.document_id == doc_id for c in admin_hits), \
        "can_read_all 管理员应可见"
