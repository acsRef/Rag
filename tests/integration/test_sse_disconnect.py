...
async def test_disconnect_background_completes(integration_db, fake_llm_stack, monkeypatch):
...
async def test_same_conversation_409(integration_db, fake_llm_stack, monkeypatch):
    ...
    with pytest.raises(HTTPException) as ei:
        await stream_chat(ChatRequest(query="再问", conversation_id=conv_id), current_user=USER)
