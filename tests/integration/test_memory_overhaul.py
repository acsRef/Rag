...
_TEST_USERS = ("hist-user", "empty-user", "loss-user", "async-user", "backoff-user", "title-user")
...
async def test_summarize_does_not_block_add_message(integration_db, monkeypatch):
    ...
    for i in range(5):
        await conversation_memory.add_message(
            conv, "user", "A%d%s" % (i, "x" * 28), user_id="async-user"
        )
...
async def test_summary_failure_backoff(integration_db, monkeypatch):
    ...
    for i in range(6):
        await conversation_memory.add_message(
            conv, "user", "B%d%s" % (i, "x" * 28), user_id="backoff-user"
        )
        await _flush()
    ...
    await conversation_memory.add_message(conv, "user", "B6%s" % ("x" * 28), user_id="backoff-user")
    await _flush()
    assert len(calls) == 4
...
async def test_title_from_first_user_message(integration_db):
    ...
    await conversation_memory.add_message(
        conv, "user", "2024年各区域销售额排名", user_id="title-user"
    )
    with get_db_ctx() as session:
