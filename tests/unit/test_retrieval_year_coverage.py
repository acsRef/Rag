...
def _make_result(chunk_id, doc_id, year, score=0.5, text="内容"):
    return {"chunk_id": chunk_id, "document_id": doc_id, "year": year,
            "score": score, "title": "", "text": text, "section_path": ""}
...
def test_supplement_appends_missing_related_years(monkeypatch):
    ...
    def fake_bm25(kb_ids, query, **kw):
        ...
        if "d2023" in doc_ids:
            return [{"chunk_id": "r23", "document_id": "d2023", "score": 0.6,
                     "title": "", "text": "2023年数据", "section_path": "2023年"}]
        return []
