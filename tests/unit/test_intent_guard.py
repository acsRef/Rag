...
def test_valid_ids_pass_through():
    out = _normalize_matches([{"kb_id": "kb-001", "score": 0.9}], KB_IDS, KB_NAMES)
    assert [(m.kb_id, m.score) for m in out] == [("kb-001", 0.9)]


def test_names_resolve_to_ids():
    out = _normalize_matches([{"kb_id": "运维手册", "score": 0.8}], KB_IDS, KB_NAMES)
    assert [m.kb_id for m in out] == ["kb-002"]


def test_unknown_kb_dropped():
    assert _normalize_matches([{"kb_id": "不存在的库", "score": 0.9}], KB_IDS, KB_NAMES) == []
...
def test_non_dict_entries_dropped():
    assert _normalize_matches(
        ...
        m.kb_id
        for m in _normalize_matches(
            ["kb-001", 42, None, {"kb_id": "kb-001", "score": 0.5}], KB_IDS, KB_NAMES
        )
    ] == ["kb-001"]


def test_bad_score_dropped():
    assert _normalize_matches([{"kb_id": "kb-001", "score": "很高"}], KB_IDS, KB_NAMES) == []


def test_numeric_string_score_coerced():
    out = _normalize_matches([{"kb_id": "kb-001", "score": "0.85"}], KB_IDS, KB_NAMES)
...
def test_whitespace_id_normalized():
    out = _normalize_matches([{"kb_id": "  kb-002 ", "score": 0.7}], KB_IDS, KB_NAMES)
    assert [m.kb_id for m in out] == ["kb-002"]
