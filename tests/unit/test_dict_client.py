            ...
            _handler(
                {
                    ...
                    ("GET", "/api/v1/kb"): httpx.Response(
                        200, json=[{"id": "kb-9", "name": "数据字典"}]
                    ),
                }
            )
        )
            ...
            _handler(
                {
                    ...
                    ("POST", "/api/v1/kb"): httpx.Response(
                        200, json={"id": "kb-new", "name": "数据字典"}
                    ),
                }
            )
        )
    ...
    def status(request):
        ...
        if state["calls"] < 2:
            return httpx.Response(
                200, json={"document_id": "d1", "status": "processing", "chunk_count": 0}
            )
        return httpx.Response(
            200, json={"document_id": "d1", "status": "indexed", "chunk_count": 3}
        )
